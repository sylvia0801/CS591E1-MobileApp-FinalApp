...
public class FavouriteRepoImpl implements FavouriteRepository {
    ...
    public void save(Favourite favourite) {
        ...
        favRef.child(favourite.getUserId()).child(k).setValue(t);
// same will update be same
    ...
    public void deleteFav(Favourite fav){
        Item t=fav.getItem();
        String k=t.getItemId();
        favRef.child(fav.getUserId()).child(k).removeValue();
    }
...
// this user has favs
                if (snapshot.getValue()!=null) {
                    ...
                    for (DataSnapshot postsanpshot : snapshot.getChildren()) {
                        ...
                        Item t=new Item(id,tagId,sellerEmail,buyerEmail,title,productName,price,description,url,address,status,postRating,rated);
                       kk.add(t);
